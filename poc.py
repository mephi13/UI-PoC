...
def main():
    ...
    with term.location(term.width-12, 4):
        bprint(term.green("*") + " " +  term.red("bob") + " ")
    ...
    messages = [
        ...
        ("System", term.orange("Connection to ") + term.red("bob") + term.orange(" established"), datetime(2022, 3 ,21, 11, 25 ,49)),
    ...
    for i, mes in enumerate(messages):
        with term.location(0, term.height - 6 - i):
            ...
            if mes[0] == user["username"]:
                ...
                message += (term.gray("[")  + term.red(mes[0]) + term.gray("]>") + mes[1])
            print(message)
